...
export function loadFromLocalStorage() {
    try {
        ...
        if (saved) {
            ...
            // Explicitly parse and restore pedal rotations for placed boards
            if (parsed.placedBoards) {
                state.placedBoards = parsed.placedBoards.map(board => ({
                    ...board,
                    pedals: (board.pedals || []).map(p => ({
                        ...p,
                        rotation: p.rotation || 0 // Falls back to 0 if legacy save data has no rotation property
                    }))
                }));
            }

            // Explicitly parse and restore pedal rotations for boardless pedals
            if (parsed.canvasPedals) {
